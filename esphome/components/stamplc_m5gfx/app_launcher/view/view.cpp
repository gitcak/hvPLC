...
static constexpr uint32_t kLauncherColorCream = 0xFFFFE0;
...
void LauncherView::_setup_option_keyframes() {
  const size_t count = _data.launcher_option_props_list.size();

  if (count == 0) {
    _data.render_order.fill(0);
    return;
  }

  if (count < 3) {
    // Not enough options for carousel animation (needs >= 3).
    // Use a simple vertical stack layout instead.
    for (size_t i = 0; i < count; i++) {
      setKeyframe(static_cast<int>(i))
          .reset(_card_ml, static_cast<int>(_card_h * i), _card_w, _card_h);
    }
    setLastKeyframe({_card_ml, 0, _card_w, _card_h});
    _data.render_order[0] = 0;
    _data.render_order[1] = count > 1 ? 1 : 0;
  ...
  for (size_t i = 0; i + 2 < count; i++) {
    setKeyframe(static_cast<int>(i + 1))
        .reset(_card_ml, _card_h + _card_h * static_cast<int>(i), _card_w,
               _card_h);
  }

  // Set the last one next to selected one to smooth the loop
  setKeyframe(static_cast<int>(count - 2))
      .reset(-130, _card_h, _card_w, _card_h);
...
void LauncherView::onOpenEnd() {
  app_open_callback(
      _data.launcher_option_props_list[getSelectedOptionIndex()].name,
...
void LauncherView::onRender() {
  // Clear
  HAL::GetCanvas()->fillScreen(kLauncherColorCream);
  ...
  for (const auto &i : _data.render_order) {
    ...
    HAL::GetCanvas()->fillTriangle(
        frame.x + _card_triangle_x0_ml, frame.y + _card_triangle_x0_mt,
    ...
    // Render icons
    if (_data.launcher_option_props_list[_data.matching_index].icon !=
        nullptr) {
      HAL::GetCanvas()->pushImage(
          frame.x + _icon_ml, frame.y + _icon_mt, _icon_w, _icon_h,
    ...
    HAL::GetCanvas()->drawString(
        _data.launcher_option_props_list[_data.matching_index].name.c_str(),
