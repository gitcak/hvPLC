...
void AppLauncher::onResume()
{
    ...
    if (_is_just_boot_up) {
        ...
        if (installed_apps.size() <= 1) {
            spdlog::warn("no launchable apps installed");
        ...
        const int max_app_index = static_cast<int>(installed_apps.size()) - 2;
        if (history_app_index < 0 || history_app_index > max_app_index) {
            history_app_index = 0;
        }
        mcAppGetFramework()->createAndStartApp(installed_apps[history_app_index + 1]);
